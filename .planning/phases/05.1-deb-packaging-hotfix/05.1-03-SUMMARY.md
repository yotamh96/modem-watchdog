...
plan: "03"
subsystem: packaging
tags: [systemd, debian, service-unit, exec-start, entry-point]

dependency_graph:
  requires:
    - "Phase 05.1 Plan 01: pyproject [project.scripts] + _sync_main + uv pip install ."
    - "Phase 05.1 Plan 02: ctl config-check verb registered at spark-modem ctl config-check"
  provides:
    - "debian/spark-modem-watchdog.service ExecStart pointing to /opt/spark-modem-watchdog/python/bin/spark-modem-watchdog"
    - "debian/spark-modem-watchdog.service ExecStartPre config-check pointing to /opt/spark-modem-watchdog/python/bin/spark-modem"
  affects:
...
tech_stack:
  added: []
  patterns:
    - "systemd unit ExecStart* paths anchored to [project.scripts] console-scripts in python/bin/ (not custom wrapper scripts)"
...
key_decisions:
  - "I-03 (executed): /opt/spark-modem-watchdog/bin/ dropped entirely; all python-script ExecStart* lines now reference /opt/spark-modem-watchdog/python/bin/ directly"
  - "L-01 (honored): LoadCredential= line preserved byte-for-byte at line 99"
  - "Phase 4 follow-up admission comment dropped; replaced with I-01/I-02/I-04 audit-trail reference"
