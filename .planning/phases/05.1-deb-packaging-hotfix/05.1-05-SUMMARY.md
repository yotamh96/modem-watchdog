...
tags: ["packaging", "ci", "testing", "smoke-test", "systemd"]
dependency_graph:
  requires:
    - "05.1-01"  # spark_modem on sys.path + console-scripts
    - "05.1-02"  # HMAC placeholder in postinst
    - "05.1-03"  # ExecStart* repointed to python/bin/
  provides:
    - "V-01 B-03 smoke extended with daemon + CLI imports"
    - "V-04 audit assertions for unit ↔ pyproject ↔ install drift"
    - "V-02 CI install test strict superset (aarch64 docker)"
  affects:
...
tech_stack:
  added: []
  patterns:
    - "V-01: extend smoke-test libs list with daemon package imports"
    - "V-04: tomllib fixture + importlib import test cross-platform"
    - "V-02: single-quote docker heredoc with outer-shell $(basename $DEB) expansion"
key_files:
  modified:
    - scripts/postinst_smoke_test.sh
    - tests/integration/test_unit_file_audit.py
    - .github/workflows/build-deb.yml
decisions:
  - "Moved tomllib and importlib imports to module top-level to satisfy ruff PLC0415"
  - "Used single-quote docker heredoc in build-deb.yml so $(basename $DEB) expands in GHA outer shell, not inside docker"
  - "install_map_dest_paths fixture parses dest column generically but installed_paths set in V-04 (a) is explicit to avoid over-engineering"
metrics:
