...
plan: 05
subsystem: tools
tags: [tools, soak-audit, phase-5, audit-soak-zao, audit-soak-exhausted, s-01, m4]

# Dependency graph
requires:
  - phase: 02-cycle-and-recovery
    provides: events.jsonl wire schema (action_planned + state_transition variants with flat usb_path)
  - phase: 02-cycle-and-recovery
    provides: Zao log RASCOW_STAT block format (parser anchor)
  - phase: 01-foundations-adrs
    provides: ADR-0003 (Zao authoritative for active lines) and ADR-0006 amendment (counter decay)
provides:
...
affects:
  - 05-07 (SOAK_RUNBOOK.md will wire both audit tools into the operator soak-exit checklist)
  - 05-07 (SIGNOFF.md template will reference the audit JSON reports as exit-gate evidence)
  - 05-08 (manual operator workflow: bench-week-end + field-2-weeks-end audit invocations)

# Tech tracking
tech-stack:
  added: []
  patterns:
    - "tools/-local _read_events_as_raw_dicts helper (mirrored across both audit scripts) — deliberately bypasses spark_modem.cli.ctl.history.read_events_with_rotated_siblings to operate over raw dicts so the Event union can evolve without breaking the audit; rationale documented in module docstrings"
    - "Forward-walk Zao RASCOW_STAT parser (audit-local) — production zao_log/parser.py only returns the LATEST snapshot via backward walk; audit needs a chronologically-sorted block list to binary-search by event ts_iso"
    - "Best-effort dynamic resolution of decay K via _resolve_decay_k_default() — tries spark_modem.policy.engine._DECAY_K_DEFAULT (forward-compat), Settings.healthy_streak_decay_k field default (current production), then literal 10 (ADR-0006 default); audit stays import-clean against current module shape"
    - "usb_path -> line derivation: trailing dotted segment of 2-3.1.N is line N (Bench hardware wires 2-3.1.{1..4}); audit refuses to fabricate a line for malformed paths and classifies them as unknown_line_derivation"

key-files:
  created:
    - tools/audit_soak_zao.py (~280 LOC)
    - tools/audit_soak_exhausted.py (~310 LOC)
    - tests/unit/tools/__init__.py (empty, new test sub-package)
    - tests/unit/tools/test_audit_soak_zao.py (6 tests, ~180 LOC)
    - tests/unit/tools/test_audit_soak_exhausted.py (6 tests, ~180 LOC)
  modified: []
