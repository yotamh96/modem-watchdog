...
plan: 01
subsystem: qmi
tags: [qmi, fleet-fixture, phase-5, parser, libqmi-version-drift, dms-get-revision]

# Dependency graph
requires:
  - phase: 02-cycle-and-recovery
    provides: QmiWrapper read-only verb scaffold (dms_get_operating_mode analog) and parser template (get_operating_mode.py)
  - phase: 02-cycle-and-recovery
    provides: per-libqmi-version fixture tree convention at tests/fixtures/qmicli/<intent>/<version>/<scenario>.txt
provides:
...
affects:
  - 05-03 (capture-fleet-fixture CLI subcommand will call dms_get_revision per-modem)
  - 05-04 (preflight_check_known_fleet_triple will call dms_get_revision + parse_get_revision)

# Tech tracking
tech-stack:
  added: []
  patterns:
    - "Parser deliberate deviation pattern: firmware/version strings preserve case (NOT lowercased) — contrast with get_operating_mode which lowercases mode='online'"
    - "tests/unit/qmi/parsers/ as a new test sub-package for parser-focused unit tests (sibling to the existing tests/unit/qmi/test_parsers.py omnibus)"
    - "Locked-set fixture-tree assertion (test_fixture_tree_has_locked_set_of_libqmi_versions) — pins the exact {1.30, 1.32} set so deletion of either is a regression"

key-files:
  created:
    - src/spark_modem/qmi/parsers/get_revision.py (56 LOC)
    - tests/fixtures/qmicli/get_revision/1.30/standard.txt (136 bytes)
    - tests/fixtures/qmicli/get_revision/1.32/standard.txt (136 bytes)
    - tests/unit/qmi/parsers/__init__.py (empty, new test sub-package)
    - tests/unit/qmi/parsers/test_get_revision.py (90 LOC, 6 tests)
    - tests/unit/qmi/test_wrapper_dms_get_revision.py (84 LOC, 2 tests)
  modified:
    - src/spark_modem/qmi/wrapper.py (added async def dms_get_revision at line 236, between dms_get_operating_mode (ends line 234) and the # ---- state-changing methods block (now at line 256))
