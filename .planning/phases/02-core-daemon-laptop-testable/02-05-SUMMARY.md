...
plan: 05
subsystem: policy
tags: [policy, recovery-spec, fr-12, fr-20, fr-21, fr-22, fr-25, fr-26, fr-26.1, adr-0006, adr-0007, adr-0008, pure-function, claude-md-1]

# Dependency graph
requires:
  - phase: 01-foundations-adrs
    provides: ModemState (5+2 ADR-0008), Diag, PlannedAction, ActionKind, IssueCategory, IssueDetail enums; Settings with backoff_seconds / ladder_min_interval_seconds / healthy_streak_decay_k / dry_run / maintenance_max_seconds
  - phase: 02-01-test-fakes-and-fixture-roots
    provides: tests/fakes/clock.FakeClock for deterministic monotonic + wall-clock arithmetic in policy/gates and policy/engine tests
provides:
...
affects: [02-06-actions, 02-07-status-prom, 02-08-webhook, 02-09-cli, 02-10-cycle-driver, phase-4-destructive-actions]

# Tech tracking
tech-stack:
  added: []
  patterns:
    - "Pure-function policy boundary: zero subprocess / httpx / asyncio / os imports in policy/ (CLAUDE.md §1; verified by regex grep + test_engine_imports_no_io_modules)"
    - "Two-Protocol seam: ClockProto co-located with PolicyContext lets policy/ accept FakeClock or a thin wrapper around production clock module without importing the module directly"
    - "Atomic cycle ordering (RECOVERY_SPEC §8): transition -> streak update -> decay check -> action select -> gates -> counter bump -> StateTransition record; all steps in-memory, single fsync per modem in plan 02-10's cycle driver"
    - "Counter bump only on action passing ALL gates including dry_run (FR-26: counters bump on execution, not selection)"
    - "Hard-skip gates (disconnected, maintenance, exhausted) short-circuit and produce skip:<reason>; soft-skip gates (signal, backoff, ladder, dry_run) accumulate into suppressed_* PlannedAction flags so events log can show partial-skip causes"
    - "Decision-table-level skip (skip:requires_human / skip:no_card / etc.) propagates to PlannedAction.reason; PlannedAction.kind is nominal SOFT_RESET because pydantic requires non-null kind"
    - "tools/check_spec.py reads _DECISION_TABLE at import time and substring-matches enum values in the spec-tests file: a 'Coverage manifest' docstring block in tests/test_recovery_spec.py guarantees discoverability"
...
key-files:
  created:
    ...
    - tools/check_spec.py
  modified:
    - tests/conftest.py  # added `settings` fixture
...
requirements-completed:
  - FR-12  # 5+2 state classification consumed end-to-end via transitions
  - FR-20  # at most one action per modem per cycle (select_top_priority_issue + decision_table)
  - FR-21  # priority order config > sim > datapath > registration > qmi (decision_table._PRIORITY_ORDER)
  - FR-22  # escalation ladder ActionKind values present (engine ladder rung selection deferred to Phase 4 destructive)
  - FR-25  # same-action backoff (300s default) -- gate_same_action_backoff
  - FR-25.1  # cross-action ladder backoff (90s default, destructive only) -- gate_ladder_backoff
  - FR-26  # counter bump on execution (engine.run_cycle step 7); decay on K consecutive healthy (step 3)
  - FR-26.1  # _healthy_streak persisted via wire alias; round-trip across model_dump_json verified by test_state_round_trips_healthy_streak_across_serialize
  - FR-26.2  # counter decay applies cleanly when prior.counters are non-empty (test_run_cycle_decay_fires_at_k_consecutive_healthy)
  - NFR-11  # policy/ never crashes the cycle: pure function + structured CycleResult; cycle driver in 02-10 wraps in try/except
  - NFR-20  # state transitions emitted as StateTransition records consumed by event_logger in plan 02-10
