...
plan: 08
subsystem: debian / systemd / logrotate
tags: [systemd, hardening, logrotate, watchdog, capabilities, sandboxing, unit-file-audit]

# Dependency graph
requires:
  - phase: 03-linux-event-sources-lifecycle
    plan: 06
    provides: SdNotifyLifecycle Type=notify wiring; WatchdogSec=90s assumed cadence; SIGTERM 5s choreography → TimeoutStopSec=10s 5s buffer; PID lock at run_dir/lock under RuntimeDirectory= (load-bearing RuntimeDirectoryPreserve=yes); LoadCredential= for HMAC secret integrates via Settings env
  - phase: 03-linux-event-sources-lifecycle
    plan: 04
    provides: EventLogWriter.reopen() contract that logrotate's postrotate signal must trigger via asyncinotify (R-01); R-02 empty-postrotate decouples log-reopen from SIGHUP config-reload (one signal verb per concern)
  - phase: 01-foundations-adrs
    provides: Phase 1 baseline systemd unit (Type=notify, ExecStartPre=postinst_smoke_test.sh, RuntimeDirectory=, LoadCredential= for HMAC secret per ADR-0011); debian/rules dh-default sequence (dh_installlogrotate auto-picks debian/spark-modem-watchdog.logrotate)
...
affects:
  - 03-09-integration-tests — Plan 03-09 wires tests/integration/conftest.py + may add per-file linux_only marker auto-add; this plan's audit test is EXEMPT from that auto-add (cross-platform by design); Plan 03-09 sets up integration suite end-to-end including the dh_installlogrotate path verification on Linux runner
  - phase 04-destructive-actions — Phase 4 destructive QMI methods consume CAP_SYS_MODULE preallocated by U-01; HIL stress tests verify WatchdogSec=90s actual fire under deliberate qmicli wedge (single unit-file edit at the start of Phase 3, no mid-rollout edits in Phase 4)
  - phase 05-bench-field-shadow — WATCHDOG cadence may be calibrated 90s ± based on real-fleet cycle_duration_seconds histograms; LoadCredential rotation runbook lives in Phase 5

# Tech tracking
tech-stack:
  added: []  # No new runtime deps; pure systemd + logrotate text + pytest text-parsing
  patterns:
    - "Cross-platform unit-file audit: parse the .service file as plain text via str.partition('=') key/value extraction; assert every U-01..U-05 directive value individually. Same precedent as tests/integration/test_default_carrier_table.py — file-parse integration tests run on every dev host, not just Linux CI."
    - "R-02 empty-postrotate decouples logrotate from SIGHUP: one signal verb per concern. The .deb owns its snippet AND its writer; the asyncinotify producer (Plan 03-04) detects the rename without needing logrotate to send a signal. logrotate handles the POSIX rotation; the daemon handles the fd swap."
    - "Phase 4-forward CapabilityBoundingSet preallocation: ship CAP_SYS_MODULE in Phase 3's unit even though Phase 3 doesn't exercise it. Single unit-file edit at the start of Phase 3, no mid-rollout edits when Phase 4 lands destructive actions."
    - "Sandboxing trade-offs documented inline (PITFALLS §4.3 + §4.4): NO PrivateMounts (LoadCredential incompat on systemd 245); NO PrivateTmp (LoadCredential compat + /run visibility for sibling ctl mutator); NO PrivateDevices (/dev/kmsg producer needs read access); RuntimeDirectoryPreserve=yes is LOAD-BEARING (clean-shutdown marker + PID lock + state.lock + modem-*.lock + metrics.sock survive systemd-supervised stop)."
...
requirements-completed: [FR-53, NFR-30]
