...
plan: 09
subsystem: testing / lifecycle integration
tags: [integration-tests, lifecycle, asyncinotify, logrotate, systemd, sigterm, sim-swap, sd-notify, flock, linux_only, phase-exit-gate]

# Dependency graph
requires:
  - phase: 03-linux-event-sources-lifecycle
    plan: 06
    provides: SdNotifyLifecycle + SigtermChoreography 8-step + classify_prior_run + PIDLock + EventSourceCrashed/SimSwapped wire variants + main.py scaffold (Plan 03-09 SC #1/#3 substrate)
  - phase: 03-linux-event-sources-lifecycle
    plan: 07
    provides: cycle_driver SIM-swap detection + StateStore.reset_modem_streak_and_counters atomic single-write (Plan 03-09 SC #2/#4 substrate)
  - phase: 03-linux-event-sources-lifecycle
    plan: 08
    provides: hardened systemd unit U-01..U-05 + R-02 logrotate snippet + cross-platform unit-file-audit gate (Plan 03-09 SC #5 substrate; audit test continues to run cross-platform — Issue #6 RESOLVED stays consistent)
  - phase: 03-linux-event-sources-lifecycle
    plan: 04
    provides: EventLogWriter.reopen() + ZaoLogInotifyTailer dual-mode + asyncinotify producer (Plan 03-09 SC #5 logrotate test substrate; FakeAsyncinotify dispatch smoke test substrate)
  - phase: 02
    plan: 10
    provides: CycleDriver pipeline + replay harness + Fake* injection roster (FakeRunner / FakeClock / FixtureInventory / FakeWebhookPoster) — directly exercised by SC #1/#2/#5 tests
...
affects:
  - "Phase 4 HIL — bench-Jetson SC #1/#3/#4/#5 hardware verification + WatchdogSec=90s actual-fire under deliberate qmicli wedge — both deferred per CONTEXT.md `Deferred Ideas → Phase 4 HIL` and tracked in STATE.md `Deferred Items` table"
  - "Phase 4 destructive actions — integration tier scaffold gives Phase 4 a stable home for new lifecycle-level integration tests as `soft_reset` / `modem_reset` / `usb_reset` / `driver_reset` land; per-module pytestmark discipline established"
  - "Phase 5 bench/field shadow — SC #1..#5 lifecycle tests serve as the regression contract that v2 must not silently break under shadow-mode dry-run; replay harness from Phase 2 + integration tier from Phase 3 form the 'no-regression' substrate"

# Tech tracking
tech-stack:
  added: []  # No new runtime deps; integration tier reuses Phase 1/2/3 production code + Fake* roster
  patterns:
    - "Integration test tier as a separate package (tests/integration/) with shared fixtures via conftest.py and Linux-gating via per-file `pytestmark = [pytest.mark.linux_only, pytest.mark.asyncio]` (NOT auto-marker collection_modifyitems — Issue #6 RESOLVED). Pattern: cross-platform tests like Plan 03-08's test_unit_file_audit.py are EXEMPT by default; opt-in to linux_only by declaring it at module level."
    - "End-to-end SC verification via Fake* injection: spawn the production substrate (CycleDriver / StateStore / EventLogWriter / SigtermChoreography / lifecycle modules) directly with FakeRunner + FakeClock + FixtureInventory + FakeSdNotify + FakePIDLock; use asyncio.Event for SIGTERM injection (NOT real os.kill signals — preserves CLAUDE.md anti-pattern: signal.signal forbidden); FakeClock-driven latency assertions for SC #1 (READY ≤60s) + SC #3 (SIGTERM ≤5s budget)."
    - "Real-binary integration test for FR-43 logrotate cron: subprocess.run(/usr/sbin/logrotate -f -s tmp_state tmp_conf) wrapped in asyncio.to_thread (ASYNC221) against a tmp-bound config matching debian/spark-modem-watchdog.logrotate; verifies dh_installlogrotate target shape end-to-end without needing a full .deb install. Per-test pytest.mark.skipif(not Path('/usr/sbin/logrotate').exists()) so dev runners without the binary skip cleanly."
    - "Bench-hardware verification deferral pattern: when hardware is not accessible at phase exit but all automatable acceptance criteria pass, ship phase with `approved-with-deferral` resume signal + a tracked Phase N+1 HIL ticket in STATE.md `Deferred Items` table. Avoids blocking phase exit on environment availability while preserving auditability."
...
requirements-completed: [FR-1, FR-43, FR-43.1, NFR-12]
...
completed: 2026-05-08T16:35:00Z
---
