...
plan: 05
subsystem: event-sources / kmsg
tags: [kmsg, classifier, dedup, issue-detail, host-issues, non-blocking-read, tdd]

# Dependency graph
requires:
  - phase: 03-linux-event-sources-lifecycle
    plan: 01
    provides: WakeSignal.KMSG closed StrEnum member; restart_on_crash supervisor (E-01); 6 host-level IssueDetail values (USB_OVERCURRENT/USB_ENUM_FAILURE/THERMAL_THROTTLE/QMI_WWAN_PROBE_FAIL/TEGRA_HUB_PSU_DROOP/UNKNOWN — D-03 closed-enum extension)
  - phase: 03-linux-event-sources-lifecycle
    plans: [03, 04]
    provides: minimal-loop producer pattern + co-located Protocol pattern + closure-factory test seam — adopted verbatim here for /dev/kmsg (no deferred Linux import needed; os.open + add_reader is stdlib)
  - phase: 02-core-daemon-laptop-testable
    provides: webhook/dedup.py DedupTable shape — KmsgDedup mirrors with key=(detail,) and default window_seconds=30.0 instead of (modem,kind)/60.0; tests/fakes/clock.FakeClock for monotonic-clock injection
...
affects:
  - 03-06-lifecycle-integration — TaskGroup wiring will spawn run_kmsg_producer alongside udev/rtnetlink/asyncinotify, each wrapped in restart_on_crash; production IssueEmitter implementation lives in 03-06 (extends event_logger writer + adds host-issue accumulator on cycle driver); sequence_gaps_total local counter surfaces as Prometheus metric kmsg_sequence_gaps_total
  - Phase 4 destructive-action gating — usb_reset suppression while USB_OVERCURRENT is the active host issue reads from this surface; THERMAL_THROTTLE and TEGRA_HUB_PSU_DROOP gate destructive actions identically

# Tech tracking
tech-stack:
  added: []  # No new dependencies — pure stdlib (asyncio, errno, os, re, contextlib)
  patterns:
    - "Tight-loop producer with classify+dedup gate (variant of 03-03/03-04 pure-passthrough): kmsg producer body classifies and dedups before push_nowait — work IS done in the loop body because /dev/kmsg cannot be coalesced (each kernel record is potentially a distinct IssueDetail; we cannot lose information by collapsing). Discipline: classify is pure (no I/O, no locks); dedup is in-memory dict access (microseconds); the loop is still constant-work-per-record, just with non-trivial constant."
    - "fd_factory tuple injection: ``fd_factory: tuple[int, Callable[[int, int], bytes]] | None = None`` — tests pass (sentinel_fd, fake.read) so the producer never opens /dev/kmsg; production wires None and ``os.open`` runs lazily inside the coroutine. Same testable-defaults pattern as Plan 03-03's ``ipr_factory``."
    - "Test path uses loop.call_soon (not loop.add_reader): fake fd is a sentinel (99) not registered with the OS event loop; tests inject all expected records up front, the producer drains them in one pass via the call_soon-scheduled callback, then BlockingIOError terminates the drain loop. Production path keeps the real loop.add_reader semantics."
    - "EPIPE-as-self-heal at the producer-level (different from 03-03's ENOBUFS-escapes-to-supervisor): EPIPE on /dev/kmsg means the reader fell behind and the kernel ring buffer wrapped — the next read after EPIPE returns the new tail of the ring. We catch EPIPE inside the drain loop, reset last_seq to None (sequence gaps from before the wrap aren't meaningful), and continue. Other OSError values escape to the supervisor (E-01) for re-entry."
    - "Re.IGNORECASE on KMSG_PATTERNS: the RESEARCH.md catalog cited capital-USB shape (``r\"USB \\S+: device not accepting address\"``); real Linux kernel writes lowercase 'usb 1-3.1: ...'. Bench-Jetson regex iteration per CONTEXT.md 'Claude's Discretion' — the IssueDetail enum is LOCKED, the regex shapes are data."
    - "KmsgDedup.should_emit returns True on EMIT (semantics flipped vs webhook/dedup.py's is_deduped which returns True on suppression). Caller phrasing: ``if dedup.should_emit(detail, ...): emit_issue(...)`` — boolean direction chosen for caller readability since the producer reads it as 'yes, push the Issue'."
...
key-files:
  ...
  modified: []  # No production-code modifications outside the new kmsg/ package + new event_sources/kmsg_producer.py module
